...
export type BuiltInTemplate = {
  _id: string; // unique key for referencing/copying
  name: string;
  description: string;
  tier: Tier;
  tags: string[];
...
function buildTrigger(i: number): BuiltInTemplate["trigger"] {
  ...
  if (t === "schedule") {
    const cron = i % 2 === 0 ? "0 9 * * 1" : "0 8 * * *"; // Monday 9:00 / Daily 8:00 UTC
    return { type: t, cron };
  }
  if (t === "webhook") {
...
function generateTemplatesForTier(tier: Tier, count = COUNT_PER_TIER): BuiltInTemplate[] {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    out.push({
      ...
      tier,
      tags: Array.from(
        new Set([cat.toLowerCase().replace(/\s+/g, "-"), pick(tags, idx), pick(tags, idx + 1)])
      ),
